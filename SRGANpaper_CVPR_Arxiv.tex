...
\kor{생성적 적대 신경망을 이용한 실사 단일 이미지 초해상도 생성}}
...
더 빠르고 깊은 합성곱 신경망을 이용한 단일 이미지 초해상도 생성 (기술에 대한) 정확도와 속도의 획기적인 약진에도 불구하고, "큰 해상도 향상 인자를 사용해서 초해상도 (이미지)를 생성할 때, 어떻게 해야 고품질 질감의 세부 사항을 복구할 수 있는가?"라는 한가지 중요한 큰 문제는 해결되지 않았다.
}\eng{
...
본 논문에서는 이미지 초해상도(SR) (생성)을 위한 생성적 적대 신경망인 SRGAN을 제시한다.
}\eng{
 ...
 우리가 아는 한, SRGAN은 $4\times$ 해상도 향상(인자)로 자연스러운 실사 이미지 추론을 할 수 있는 첫번째 프레임워크이다.
}\eng{
...
초해상도 (생성) 이미지와 원본 실사 이미지를 구별하도록 훈련된 판별기(discriminator) 네트워크를 이용해서 적대적 손실은 해(solution)를 자연 이미지 데이터 공간(manifold)로 밀어넣는다.
}\eng{
 In addition, we use a content loss motivated by perceptual similarity instead of similarity in pixel space.
}\kor{
 추가적으로, 픽셀 공간의 유사성 대신 지각적 유사성으로부터 영감를 받은 내용 손실을 사용한다.
}\eng{
...
고해상도 \ac{HR} 이미지에 대응하는 저해상도 \ac{LR} 이미지로부터 고해상도 \ac{HR} 이미지를 추정하는 매우 도전적인 작업을 초해상도 (생성) \ac{SR}이라고 한다.
}\eng{
...
초해상도 (생성) \ac{SR}은 컴퓨터 비전 연구 공동체 내에서 상당한 관심을 받았으며, 광범위하게 응용되고 있다 \cite{yang2007spatial,Zou12,Nasrollahi2014}.
}
...
초해상도 (생성된) 이미지(좌측)는 원본(우측)과 거의 구분이 불가능하다. [$4\times$ 해상도 향상]
}}
...
불량 조건인(LR을 HR로 복원하는 경우 정답이 1개가 아닌 여러개가 존재하는 것을 불량 조건을 가진다고 함) 미결정 초해상도 (생성) 문제의 본질은 특히 일반적으로 재구성된 SR 이미지의 질감 세부 사항이 없는 높은 해상도 향상 인자에서 두드러진다.
}
...
왼쪽에서 오른쪽 순으로: 쌍삼차(bicubic) 보간, MSE로 최적화된 심층 잔류 신경망, 원본 고해상도(HR) 이미지에 대해서 조금 더 사람의 지각에 민감한 손실로 최적화된 심층 잔류 생성적 적대 신경망. 해당 PSNR과 SSIM은 괄호 안에 표시됨. [$4\times$ 해상도 향상]
}
}
...
지도 \ac{SR} 알고리즘의 최적화 목표는 보통 고해상도(HR) 이미지와 정답 사이의 \ac{MSE}를 최소화하는 것이다.
}\eng{
...
\ac{MSE}를 최적화하는 것은 \ac{SR} 알고리즘을 평가하고 비교하는데 일반적으로 사용되는 \ac{PSNR}을 최대화하기 때문에 편리하다.\cite{Yang14benchmark}
}
\eng{
...
하지만, MSE(그리고 PSNR)의 높은 질감 세부 정보와 같은 지각적으로 관련된 차이를 포착하는 능력은 픽셀 단위의 이미지 차이를 기반으로 정의되었기 때문에 매우 제한적이다.
}\eng{
 ...
 이것은 높은 PSNR이 필연적으로 지각적으로 더 높은 \ac{SR} 결과를 반영하지 않는 (경우인) \figurename~\ref{fig:exampleIntro}에서 보여진다.
}\eng{
...
본 논문에서는 건너 뛴 연결(skip-connection)을 사용하고 단일 최적화 목표로부터 \ac{MSE}를 분리한 심층 \ac{ResNet}을 이용한 \ac{SRGAN}을 제안한다.
 ...
 이전 연구와 달리, 고해상도 \ac{HR} 참조 이미지와 지각적으로 구분하기 어려운 해들(solutions)을 부추기는 판별기(discriminator)와 결합한 VGG 네트워크 \cite{simonyan2014very,Johnson16PercepLoss,bruna2016super}의 고수준 특징 맵을 사용해서 새로운 지각 손실을 정의한다.
}
\eng{
...
$4\times$ 해상도 향상 인자로 초해상도 (생성한) 예제 실사 이미지는 \figurename~\ref{fig:exampleIntroFirst}에서 보여진다.
}

\subsection{관련 연구}
\subsubsection{이미지 초해상도 (생성)}
...
이미지 \ac{SR} (분야의) 최근 개요적 기사에는 Nasrollahi와 Moeslund \cite{Nasrollahi2014} or Yang 외가 있다.
}\eng{
...
본 논문에서는 \ac{SISR}에 초점을 맞추고, 고해상도 \ac{HR} 이미지들을 여러 이미지로부터 복원하는 \cite{Borman1998aSurvey,Farsiu2004} 접근에 대해서는 더 논의하지 않을 것이다.
}
...
예측 기반 방법은 \ac{SISR}을 해결하려고 한 첫번째 방법 중 하나였다.
} \eng{
...
선형, 쌍삼차(bicubic), Lanczos \cite{Duchon1979} 필터링과 같은 필터링 접근법들은 매우 빠를수 있지만, \ac{SISR} 문제를 지나치게 단순화하고, 일반적으로 과하게 부드러운 질감을 가지는 해(solution)을 산출한다.
}
\eng{
...
In Dai et al. \cite{dai2015jointly} a multitude of patch-specific regressors is learned and the most appropriate regressors selected during testing. %. During testing the most appropriate regressors for a given \ac{LR} patch are then selected using kNN.
...
This is an improvement over Dong et al. \cite{dong2016image} where bicubic interpolation is employed to upscale the LR observation before feeding the image to the \ac{CNN}. %In addition, by extracting the feature maps in \ac{LR} space as in \cite{Shi2016ESPCN, Johnson16PercepLoss, dong2016accelerating}, the gain in speed can be used to employ a deep \ac{ResNet} to increase accuracy.
