...
\newacronym{평균 의견 점수}{평균 의견 점수}{mean opinion score}
...
더 빠르고 깊은 합성곱 신경망을 사용하는 단일 이미지 초해상도화 기술에 대한 정확도와 속도의 획기적인 약진에도 불구하고, ``큰 해상도 향상 계수를 사용해서 초해상도 이미지를 생성할 때, 어떻게 해야 고품질 질감의 세부 사항을 복구할 수 있는가?''라는 중요한 문제는 해결되지 않았다.
}\eng{
...
최적화 기반 초해상도 방법의 동작은 주로 목적 함수를 선택하는 것에 의해 좌우된다. 최근 연구는 평균 제곱 재구성 오류를 최소화하는 것에 크게 초점이 맞춰져 있다.
}\eng{
...
본 논문에서는 이미지 초해상도화를 위한 생성적 적대 신경망인 SRGAN을 제시한다.
 ...
 우리가 아는 한, SRGAN은 $4\times$ 해상도 향상 계수로 자연스러운 실사 이미지 추론을 할 수 있는 첫번째 프레임워크이다.
}\eng{
...
초해상도화 이미지와 원본 실사 이미지를 구별하도록 훈련된 판별기(discriminator) 네트워크를 이용해서 적대적 손실은 해(solution)를 자연 이미지 데이터 공간(manifold)으로 밀어넣는다.
}\eng{
...
본 논문에서 제시하는 심층 잔류 신경망은 공개 벤치마크에서 심하게 다운샘플링된 이미지로부터 실사와 같은 질감을 복원해낼 수 있다.
}\eng{
...
\summary{
이 논문은 SRGAN이라는 생성적 적대 신경망을 제안하며, 고해상도 이미지의 질감을 복원하는 문제를 해결함. SRGAN은 적대적 손실과 내용 손실로 구성된 지각 손실 함수를 사용하며, 대규모 평가에서 높은 평균 의견 점수를 받음. 이를 통해 SRGAN이 최첨단 방법보다 원본 고해상도 이미지에 가까운 결과를 보인다는 것을 확인함.
}
...
왼쪽에서 오른쪽 순으로: 쌍삼차(bicubic), MSE로 최적화된 심층 잔류 신경망, 원본 고해상도 이미지에 대해서 조금 더 사람의 지각에 민감한 손실로 최적화된 심층 잔류 생성적 적대 신경망. PSNR과 SSIM은 괄호 안에 표시된다. [$4\times$ 해상도 향상]
}
}
...
지도(supervised) \ac{초해상도화} 알고리듬의 최적화 목표는 보통 \ac{고해상도} 이미지와 정답 사이의 \ac{MSE}를 최소화하는 것이다.
}\eng{
...
\ac{MSE}를 최적화하는 것은 \ac{초해상도화} 알고리듬\cite{Yang14benchmark}을 평가하고 비교하는데 일반적으로 사용되는 \ac{PSNR}을 최대화하기 때문에 편리하다.
}
\eng{
...
초해상도 (생성물)과 원본 이미지 사이의 지각적 차이는, 복원한 이미지는 Ferwerda \cite{Ferwerda2003}에서 정의된 것처럼 실사적이지 않다는 것이다.
}

\summary{
높은 해상도 향상 계수에서의 초해상도화는 질감 세부 사항이 부족하며, MSE와 PSNR을 최적화 목표로 삼는 경우 높은 질감 세부사항과 같이 지각적으로 관련된 차이를 포착하는 능력이 제한적임.
}
...
\summary{
CNN 기반 초해상도화, LISTA 알고리즘, DRCN, 지각적 유사성 손실 함수 등 초해상도화 분야의 신경망을 이용한 최근 기술과 방법 소개
}
...
한편, 많은 컴퓨터 비전 문제에 대한 최첨단 방법은 Krizhevsky 외. \cite{krizhevsky2012imagenet} 작업의 성공의 따라, 특별히 설계된 \ac{합성곱 신경망} 구조에 의해 설정된다.
}

\summary{컴퓨터 비전의 주류 최첨단 방법인 CNN}
...
심층 네트워크 구조는 훈련하기 어려울 수 있지만, 매우 높은 복잡성의 모델링 사상(mapping)을 허용하므로 네트워크의 정확도를 충분히 높일 수 있는 잠재력이 있는 것을 보여준다.
} \eng{
...
\summary{심층 네트워크 구조, 배치 정규화, 잔차 블록과 건너뛴 연결 등의 방법과 구조는 초해상도화 성능 향상에 있어 중요함.}
...
\summary{
쌍삼차 보간을 이용한 방법보다 정확도와 속도 면에서 개선된 방법인 해상도 필터 학습 방법
}
...
MSE 기반 방법은 픽셀 공간에서 가능한 해의 픽셀 단위(pixel-wise) 평균으로 인해 과하게 매끄럽게 보이는 반면, GAN은 자연 이미지 데이터 공간(manifold)쪽으로 재구성을 유도해서, 지각적으로 더 실감나는 해를 생성한다.
}}
\label{fig:manifold}
\end{figure}
...
} %Specifically, we employ the fast feature learning in \ac{LR} space \cite{Shi2016ESPCN,Johnson16PercepLoss} and batch-normalization \cite{Ioffe2015} to robustly train a network of 16 residual blocks for better accuracy.
\item \eng{
...
} %as illustrated in Figures 3 and 4 in Li and Wand .
\item \eng{
...
~~~~~\textbf{SRResNet} & ~~~~~~~SRGAN-MSE & ~~SRGAN-VGG22 & ~~\textbf{SRGAN}-VGG54 & 원본 고해상도 이미지~~~ \\
}
\end{tabular}
...
\zoombox[magnification=5,color code=lime]{0.86,0.25}
\end{tikzpicture}
\caption{\eng{\textbf{SRResNet} (left: a,b), SRGAN-MSE (middle left: c,d), SRGAN-VGG2.2 (middle: e,f) and  \textbf{SRGAN}-VGG54 (middle right: g,h) reconstruction results and corresponding reference HR image (right: i,j). [$4\times$ upscaling]} \kor{
\textbf{SRResNet} (좌측: a,b), SRGAN-MSE (중앙 좌측: c,d), SRGAN-VGG2.2 (중앙: e,f), \textbf{SRGAN}-VGG54 (중앙 우측: g,h) 재구성 결과와 참조 고해상도 이미지 (우측: i,j). [$4\times$ 해상도 향상]}}
...
} %A deeper investigation of the network architecture as well as the perceptual loss will be part of future work.
\eng{
