...
이러한 이미지들은 테스트 이미지들과는 다르다. 본 논문에서는 $r=4$인 다운샘플링 계수를 가지는 쌍삼차 보간(bicubic) 커널을 사용하여 \ac{고해상도} 이미지 (BGR, $C=3$)를 다운샘플링하여 \ac{저해상도} 이미지를 얻었다.
} \eng{
...
수행된 \ac{평균 의견 점수} 테스트의 실험 결과는 표 \ref{tab:perceptual}, 표 \ref{tab:performance}와 \figurename~\ref{fig:MOS}에 요약되어 있다.
}
