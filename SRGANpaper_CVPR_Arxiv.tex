...
본 논문에서는 \ac{단일 이미지 초해상도화}를 위한 지각적 손실 함수를 형성하기 위해 \acp{GAN}의 개념을 사용하는 최초의 \ac{ResNet} \cite{he2015deep,he2016identity} 아키텍처를 설명한다.
}
\begin{itemize}
\item \eng{
...
우리는 새로운 지각 손실에 최적화된 GAN 기반 네트워크인 SRGAN을 제안한다. 여기서 우리는 MSE 기반 콘텐츠 손실을 픽셀 공간 Li2016의 변화에 더 불변하는 VGG 네트워크 simonyan2014very의 기능 맵에서 계산된 손실로 대체한다.
새로운 지각 손실에 최적화된 \ac{GAN} 기반 네트워크인 SRGAN을 제안한다. 여기서 우리는 \ac{MSE} 기반 내용 손실
