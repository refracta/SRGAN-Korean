...
최근에는 \ac{합성곱 신경망} 기반 \ac{초해상도} 알고리즘이 우수한 성능을 보여주고 있다.
}
\eng{
...
Dong 외. \cite{dong2014learning,dong2016image}는 입력 이미지를 업스케일하기 위해 쌍삼차 보간(bicubic interpolation)을 사용하고, 최첨단 \ac{초해상도화} 성능을 달성하기 위해 세 개의 훈련된 레이어를 가지는 완전한 심층 합성곱 신경망을 종단 간(end-to-end) 훈련시켰다.
}
\eng{
...
그 결과로, 네트워크가 업스케일링 필터를 직접 학습할 수 있게 하면 정확도와 속도 측면에서 성능을 더욱 향상시킬 수 있다는 것을 보여주었다.
}
...
-텍스처 세부 정보가 높은 여러 잠재적 솔루션이 평균화되어 원활한 재구성을 만드는 fig:manifold에서 MSE을 최소화하는 문제를 설명한다.
