...
초해상도 (생성물)과 원본 이미지 사이의 지각적 차이는, 복구한 이미지는 Ferwerda \cite{Ferwerda2003}에서 정의된 것처럼 실사적이지 않다는 것이다.
}
...
본 논문에서는 \ac{단일 이미지 초해상도화}에 초점을 맞추고, 고해상도 \ac{고해상도} 이미지들을 여러 이미지로부터 복구하는 \cite{Borman1998aSurvey,Farsiu2004} 접근에 대해서는 더 논의하지 않을 것이다.
}
...
이 작업과 유사하게, Johnson 외. \cite{Johnson16PercepLoss}와 Bruna 외. \cite{bruna2016super}는 저수준의 픽셀 단위 에러 측정 대신 사전 훈련된 VGG 네트워크로부터 추출된 특징을 사용하는 것을 제안한다.
} \eng{
...
본 논문에서는 \ac{단일 이미지 초해상도화}를 위한 지각적 손실 함수를 형성하기 위해 \acp{GAN}의 개념을 사용하는 최초의 \ac{ResNet} \cite{he2015deep,he2016identity} 구조를 설명한다.
}
\begin{itemize}
\item \eng{
...
본 연구에서는 복구된 \ac{초해상도화} 이미지의 명확하고 바람직한 특성을 만들어내는 여러 손실 구성 요소의 가중 조합(weighted combination)으로 지각 손실 $l^{SR}$을 구체적으로 설계할 것이다.
} \eng{
